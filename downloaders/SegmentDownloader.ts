...
export class SegmentDownloader {
   ...
   */
  public async downloadSegments(segmentUrls: string[], outputPrefix: string = 'segment'): Promise<string[]> {
   ...
   */
  private async downloadSegment(url: string, outputPath: string, index: number): Promise<boolean> {
    for (let attempt = 1; attempt <= this.config.retryAttempts!; attempt++) {
      try {
        ...
        console.error(`  Segment ${index + 1} attempt ${attempt} failed:`, (error as Error).message);
   ...
   */
  public getDownloadedSegments(): string[] {
   ...
   */
  public getProgress(): { downloaded: number; total: number; percentage: number } {
   ...
   */
  public async downloadM3U8WithCurl(url: string, headers: Record<string, string>): Promise<string | null> {
    try {
   ...
   */
  public async downloadSegmentsWithCurl(segmentUrls: string[], headers: Record<string, string>, outputPrefix: string = 'segment'): Promise<string[]> {
    ...
    for (let i = 0; i < segmentUrls.length; i += this.config.maxConcurrentDownloads!) {
      ...
      const batchResults = await Promise.all(batchPromises);
      const successfulPaths = batchResults.filter(path => path !== null) as string[];
   ...
   */
  private async downloadSegmentWithCurl(url: string, outputPath: string, headers: Record<string, string>): Promise<boolean> {
