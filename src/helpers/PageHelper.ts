...
export class PageHelper extends BaseHelper {
  ...
  async waitForJWPlayerInitialization(page: Page): Promise<void> {
    ...
    try {
      await page.waitForSelector('script[src*="jwplayer"]', { timeout: 10000 });
      this.logger.info('✅ JWPlayer script detected');
    } catch {
      this.logger.warn('⚠️ JWPlayer script not found, continuing anyway...');
    }
    ...
    try {
      const jwplayerReady = await page.evaluate(() => {
        return typeof (window as any).jwplayer !== 'undefined' && 
  ...
  async extractVideoUrlsFromDOM(page: Page): Promise<any[]> {
    try {
      ...
      for (let frameIndex = 0; frameIndex < allFrames.length; frameIndex++) {
        ...
        try {
          this.logger.info(`🔍 Checking ${frameName} frame for video elements...`);
          ...
          if (videoElements && videoElements.length > 0) {
            this.logger.info(`🎬 FOUND ${videoElements.length} VIDEO URLs IN ${frameName.toUpperCase()}:`);
            
            for (const videoInfo of videoElements) {
              this.logger.info(`  🎯 ${videoInfo.type} (${videoInfo.element}): ${videoInfo.url}`);
          ...
          this.logger.debug(`Error checking frame ${frameIndex}: ${error}`);
        }
      }
  ...
  async takeScreenshot(page: Page, filename?: string): Promise<string | null> {
    try {
  ...
  async scrollToElement(page: Page, selector: string): Promise<boolean> {
    try {
      ...
      this.logger.debug(`Could not scroll to element ${selector}: ${error}`);
      return false;
    }
  }
  ...
  async clickElement(page: Page, selector: string, timeout: number = 3000): Promise<boolean> {
    try {
      const element = page.locator(selector);
