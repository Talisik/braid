...
import { M3U8Processor } from './M3U8Processor';
...
export class StreamHandler {
  ...
  async processResults(candidates: VideoCandidate[]): Promise<boolean> {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      this.logger.info(`  ${i + 1}. ${candidate.domain} (age: ${age.toFixed(1)}s, source: ${source})`);
    ...
    for (let i = 0; i < sortedCandidates.length; i++) {
      ...
      this.logger.info(`Trying candidate ${i + 1}/${sortedCandidates.length} (score ${score}): ${url}`);

      // Skip heavily penalized ad domains
      if (score < 0) {
        this.logger.warn(`SKIPPING AD CANDIDATE ${i + 1} (score ${score}): ${url}`);
        continue;
      }

      // Skip very low scoring candidates that are likely ads
      if (score < 50) {
        this.logger.warn(`SKIPPING LOW-QUALITY CANDIDATE ${i + 1} (score ${score}): ${url}`);
        continue;
      }
      ...
      if (this.isDirectVideoUrl(url)) {
        success = await this.processor.downloadDirectVideo(url, candidate.headers);
      } else {
        success = await this.processor.processM3U8(url, candidate.headers);
      }

      if (success) {
        this.logger.info(`SUCCESS! Downloaded video using candidate ${i + 1}`);
        return true;
      } else {
        this.logger.warn(`Candidate ${i + 1} failed (likely contains ads), trying next...`);
      }
    }
  ...
  private calculateCandidateScore(candidate: VideoCandidate): number {
    ...
    if (this.isDirectVideoPlayerUrl(url)) {
      score += 5000;
      this.logger.info(`DIRECT VIDEO PLAYER URL BONUS: +5000 points for ${url}`);
    }
    ...
    if (source.includes('dom_extraction')) {
      ...
      score += 1000;
      this.logger.info(`PLAYER PAGE ANALYSIS BONUS: +1000 points for ${url}`);
    }
    ...
    if (url.toLowerCase().includes('.m3u8')) {
      ...
      if (url.includes('/master/') || url.includes('master.m3u8') || 
        ...
        score += 900;
        this.logger.info(`M3U8 MASTER MANIFEST DETECTED - HIGHEST PRIORITY: ${url}`);
      } else {
        score += 700;
        this.logger.info(`M3U8 STREAM DETECTED - HIGH PRIORITY: ${url}`);
      }
      ...
      if (url.includes('_h264_') && (url.includes('_init_') || /\d+_[a-zA-Z0-9]+_\d+\.mp4$/.test(url))) {
        ...
        this.logger.info(`HLS SEGMENT DETECTED - MEDIUM PRIORITY: ${url}`);
      } else {
        score += 600; // Complete video files still get good priority
        this.logger.info(`DIRECT VIDEO FILE DETECTED - HIGH PRIORITY: ${url}`);
      }
    }
    ...
    if (this.isKnownStreamingDomain(domain)) {
      score += 300;
      this.logger.info(`KNOWN STREAMING DOMAIN - MEDIUM PRIORITY: ${url}`);
    }
  ...
  private isKnownStreamingDomain(domain: string): boolean {
    const domains = [
      'sacdnssedge.com', 'turbovidhls.com', 'streamhihi.com',
      'ovaltinecdn', 'equityvypqjdgkbw', 'tnmr.org'
    ];
  ...
  private isNonVideoUrl(url: string): boolean {
    const nonVideoPatterns = [
      '/cdn-cgi/', '/api/', '/config', '.json', '.js', '.css',
      'speculation', 'googletagmanager', 'analytics', 'tracking',
      'fonts.gstatic', 'tiktokcdn'
    ];
  ...
  private async isLikelyAdStream(url: string, headers: Record<string, string>): Promise<boolean> {
    try {
      ...
      const adIndicators = [
        ...
        'ads.yahoo', 'amazon-adsystem', 'adnxs.com', 'creative.mnaspm.com'
      ];
      
      const urlLower = url.toLowerCase();
      return adIndicators.some(indicator => urlLower.includes(indicator));
