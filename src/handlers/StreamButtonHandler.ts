...
export class StreamButtonHandler {
  ...
  async tryStreamButtonsSequentially(page: Page): Promise<boolean> {
    ...
    for (let i = 0; i < streamButtonSelectors.length; i++) {
      ...
      try {
        this.logger.info(`Trying stream button ${i + 1}/${streamButtonSelectors.length}: ${selector}`);
        ...
        if (count === 0) {
          this.logger.info(`No buttons found with selector: ${selector}`);
          continue;
        }
        ...
        for (let j = 0; j < count; j++) {
          try {
            ...
            if (page.isClosed()) {
              this.logger.warn('Page is closed, stopping button attempts');
              return false;
            }
            ...
            this.logger.info(`Clicking button: ${buttonText} (${dataLocalize})`);
            ...
            try {
              ...
              this.logger.warn(`Failed to click button ${buttonText}: ${clickError}`);
              continue;
            }

          } catch (error) {
            this.logger.warn(`Failed to click button ${j + 1} with selector ${selector}: ${error}`);
            continue;
          }
        }
  ...
  async clickSpecificStreamButton(page: Page, selector: string): Promise<boolean> {
    try {
      this.logger.info(`Attempting to click specific stream button: ${selector}`);
      ...
      this.logger.error(`Failed to click specific stream button ${selector}: ${error}`);
      return false;
    }
  }
      ...
      try {
        ...
        if (count > 0) {
          ...
          if (isVisible) {
            ...
            this.logger.info(`Found available stream button: ${buttonText} (${selector})`);
          }
        }
      } catch (error) {
        this.logger.debug(`Error checking selector ${selector}: ${error}`);
      }
    }

    this.logger.info(`Found ${availableButtons.length} available stream buttons`);
