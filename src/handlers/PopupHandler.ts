...
export class PopupHandler {
        ...
        if (count > 0) {
          for (let i = 0; i < count; i++) {
            try {
              ...
              this.logger.info(`Clicking popup close button: ${selector}`);
              ...
              this.logger.debug(`Failed to click popup element ${i} with selector ${selector}: ${error}`);
            }
          }
        }
      } catch (error) {
        this.logger.debug(`Error with popup selector ${selector}: ${error}`);
      }
    }

    // Also handle page dialogs (alert, confirm, prompt)
    page.on('dialog', async (dialog) => {
      this.logger.info(`Handling dialog: ${dialog.type()} - ${dialog.message()}`);
  ...
  async closeSpecificPopup(page: Page, selector: string): Promise<boolean> {
    try {
      ...
      this.logger.debug(`Failed to close specific popup ${selector}: ${error}`);
      return false;
    }
  }

  async waitAndClosePopups(page: Page, timeout: number = 5000): Promise<void> {
    this.logger.info(`Waiting ${timeout}ms for popups to appear and closing them...`);
