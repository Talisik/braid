export class StreamFilter {
    ...
    getVideoStreams(requests) {
        ...
        return requests.filter(request => videoPatterns.some(pattern => pattern.test(request.url)));
    }
    /**
     * Filter and analyze video streams with quality detection
     */
    filterVideoStreams(requests) {
        ...
        const streams = videoStreams.map(request => {
    ...
    extractQuality(url) {
        ...
        if (urlLower.includes('4k') || urlLower.includes('2160p'))
            return 2160;
        if (urlLower.includes('1080p') || urlLower.includes('fhd'))
            return 1080;
        if (urlLower.includes('720p') || urlLower.includes('hd'))
            return 720;
        if (urlLower.includes('480p'))
            return 480;
        if (urlLower.includes('360p'))
            return 360;
        if (urlLower.includes('240p'))
            return 240;
     ...
     */
    getM3U8Streams(requests) {
        return requests.filter(request => request.url.toLowerCase().includes('.m3u8'));
    }
    /**
     * Get MP4 direct video URLs
     */
    getMP4Streams(requests) {
        return requests.filter(request => request.url.toLowerCase().includes('.mp4'));
    }
    /**
     * Get streaming manifest files (M3U8, MPD)
     */
    getManifestStreams(requests) {
        return requests.filter(request => {
