...
export class F2MoviesDL {
    ...
    try {
      ...
      try {
        ...
        const videoRequests = requests.filter(req => 
          req.url.includes('.m3u8') || req.url.includes('video') || req.url.includes('stream')
        ).length;
      ...
      try {
        ...
        console.log('Browser already closed or error closing:', (error as Error).message);
      }
   ...
   */
  private async analyzeM3U8Content(url: string, size: number): Promise<boolean> {
    try {
      ...
      console.log(`      Failed to analyze M3U8 content: ${(error as Error).message}`);
   ...
   */
  private async handleCriticalPopups(page: Page): Promise<void> {
    try {
   ...
   */
  public async findVideoStreams(
    url: string,
    monitorDuration: number = 20000,
    clickPlay: boolean = true
  ): Promise<any[]> {
    try {
   ...
   */
  public async testPlayButtonDetection(url: string): Promise<boolean> {
